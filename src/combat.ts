import { CombatResource } from "grimoire-kolmafia";
import {
  equippedItem,
  Item,
  Monster,
...
import { BanishSource, FreekillSource, RunawaySource, WandererSource } from "./engine/resources";
...
export class CombatResourceAllocation {
  ...
  public getMacro(for_strategy: MonsterStrategy): Macro | undefined {
    ...
    throw `Unable to convert resource ${resource} to a macro`;
  }
}
...
export class BuiltCombatStrategy {
  ...
  prepare_macro(strategy: MonsterStrategy | Macro, monster?: Monster): Macro {
    ...
    const delevel = new Macro()
      ...
      .tryItem([$item`HOA citation pad`, $item`Mayor Ghost's scissors`])
      .tryItem($item`Zombo's empty eye`);
